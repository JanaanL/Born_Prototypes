...
int prop_blocked(float* p0, const float* p1, float *vel){ $
	born_profiler_t profiler;
	profiler.start();
	...
	for(int ii3=4; ii3 < _nz - 4; ii3+=stride){
		for(int ii2=4; ii2 < _ny - 4; ii2+=stride) {
		   for (int ii1 = 4; ii1 < _nx - 4; ii1+=stride){
			tbb::parallel_for(tbb::blocked_range<int>(ii3,std::min(ii3+stride,_nz-4)),
			[&] (const tbb::blocked_range<int>&r){
			for (int i3 = r.begin(); i3 != r.end(); ++i3){
		    	    for (int i2 = ii2; i2 < std::min(_ny-4,ii2+stride); i2++) {
				int ii = i2*stride +  stride*stride*i3 + ii1;
			    for(int i1=ii1; i1 < std::min(ii1+stride, _nx-4); i1++, ii++) {
...
int vector_prop(float* p0, const float* p1, float *vel) { $ 
	...
	int stridexy = STRIDEXY;
	int stridez = STRIDEZ;
	for(int ii3=4; ii3 < _nz - 4; ii3+=stridez){
		for(int ii2=4; ii2 < _ny - 4; ii2+=stridexy) {
		   for (int ii1 = 4; ii1 < _nx - 4; ii1+=stridexy){
			tbb::parallel_for(tbb::blocked_range<int>(ii3,std::min(ii3+stridez,_nz-4)),
			[&] (const tbb::blocked_range<int>&r){
			for (int i3 = r.begin(); i3 != r.end(); ++i3){
		    	    for (int i2 = ii2; i2 < std::min(_ny-4,ii2+stridexy); i2++) {
				int idx = i2*stridexy +  stridez*stridexy*i3 + ii1;
			    for(int i1=ii1; i1 < std::min(ii1+stridexy, _nx-4); i1+=AVX_SIMD_LENGTH) {
	...
	__m256 _zm1, _zm2, _zm3, _zm4, _zp1, _zp2, _zp3, _zp4;
				...
				//load center values
				_p1 = _mm256_loadu_ps( p1 + idx);
				_p0 = _mm256_loadu_ps( p0 + idx);
				_vel = _mm256_loadu_ps( vel + idx); 
				//_center = _p1;		
				...
				_o = _mm256_fmadd_ps(_coeff0, _p1, _o);
...
/**blocked tiling*/
int prop_tile_block(float* p0, const float* p1, float *vel){ $
	std::cout << "Calling the prop_tile_block function" << std::endl;
	born_profiler_t profiler;
	profiler.start();

  int stride = 128;
  tbb::parallel_for(tbb::blocked_range3d<int>(4,_nz-4, stride, 4, _ny-4, stride, 4, _nx-4, stride),
    [&](const tbb::blocked_range3d<int>&r){
    for(int  ii3=r.pages().begin(); ii3<r.pages().end(); ++ii3){
	for(int ii2=r.rows().begin(); ii2<r.rows().end(); ++ii2) {
	    for(int ii1=r.cols().begin(); ii1<r.cols().end(); ++ii1){
		for (int i3 = ii3; i3 < std::min(_nz-4,ii3+stride); i3++){
		    for (int i2 = ii2; i2 < std::min(_ny-4,ii2+stride); i2++){
			int ii=i2*stride + stride*stride*i3 + ii1;
			for(int i1=ii1; i1 < std::min(_nx - 4,ii1+stride); i1++,ii++) {
				p0[ii] = vel[ii] * (
					coeffs[0]*p1[ii]
					+coeffs[1]*(p1[ii-1]+p1[ii+1])+
					+coeffs[2]*(p1[ii-2]+p1[ii+2])+
