...
int prop(float* p0, float* p1, float *vel){ $
	for(int i3=0; i3 < _nz; i3++){
		for(int i2 = 0; i2 < _ny; i2++) {
			int ii=i2*_nx  + _n12*i3;
			for(int i1 = 0; i1 < _nx; i1++,ii++) {
				p0[ii] = vel[ii] * (
					coeffs[0]*p1[ii]
...
int vector_prop(float* p0, float* p1, float *vel) { $ 
	...
	for (int i3 = 0; i3 < _nz ; i3++) {
	    for (int i2 = 0; i2 < _ny; i2++) {
		int idx = i2*_nx +_n12*i3;
			for (int i1=0; i1 < _nx; i1+=AVX_SIMD_LENGTH){
	...
	//		printf(" Finished Loading x values  into vectors\n");
				...
				//y values
		//		_ym1 = _mm256_loadu_ps( p1 - 1 * _nx);
		//		_ym2 = _mm256_loadu_ps( p1 - 2 * _nx);
		//		_ym3 = _mm256_loadu_ps( p1 - 3 * _nx);
		//		_ym4 = _mm256_loadu_ps( p1 - 4 * _nx);
		//		_yp1 = _mm256_loadu_ps( p1 + 1 * _nx);
		//		_yp2 = _mm256_loadu_ps( p1 + 2 * _nx);
		//		_yp3 = _mm256_loadu_ps( p1 + 3 * _nx);
		//		_yp4 = _mm256_loadu_ps( p1 + 4 * _nx);
		//		
	//		printf(" Finished Loading y values  into vectors\n");
				//x values
		//		_zm1 = _mm256_loadu_ps( p1 - 1 * _n12);
		//		_zm2 = _mm256_loadu_ps( p1 - 2 * _n12);
		//		_zm3 = _mm256_loadu_ps( p1 - 3 * _n12);
		//		_zm4 = _mm256_loadu_ps( p1 - 4 * _n12);
		//		_zp1 = _mm256_loadu_ps( p1 + 1 * _n12);
		//		_zp2 = _mm256_loadu_ps( p1 + 2 * _n12);
		//		_zp3 = _mm256_loadu_ps( p1 + 3 * _n12);
		//		_zp4 = _mm256_loadu_ps( p1 + 4 * _n12);
...
int main(int argc, char* argv[]) {
  ...
  coeffs[12] = 12;
   ...
   //printf("Allocating and aligning memory for arrays\n");

	// malloc, new,   intel aligned malloc 
   p0 = (float *)_mm_malloc((n + OFFSET)*sizeof(float), align);
   p1 = (float *)_mm_malloc((n + OFFSET)*sizeof(float), align);
   vel = (float *)_mm_malloc((n + OFFSET)*sizeof(float), align);
   p0_copy = (float *)_mm_malloc((n + OFFSET)*sizeof(float), align);


//p0+=4; p1+=4; vel+=4; p0_copy+=4;

	//printf("Filling in arrays with random numbers \n");
	//fill in arrays
...
//p0+=4; p1+=4; vel+=4; p0_copy+=4;
   ...
   //p0+=offset; p1+=offset; vel+=offset; p0_copy+=offset;
	...
	//printf("Printing values from original array\n");
	//for (int k = 0; k < 12; k++)
	//	printf("p0[%d]: %f ", k, p0[k]);
	//printf("\n");
//	printf("Calling check accuracy function \n");i
	//prop(p0_copy, p1, vel);
	prop(p0_copy, p1, vel);
